...
We next show that the maps $Lf^*$ and $f_*$ interact well with the functors $\Phi$ and $\Phi_f$.
...
Let $\kk$ be an infinite field.  Let $R$ be a finite, graded extension of $S$ and assume that $\Proj(R)$ contains an Ulrich sheaf.  Then, for any codimension sequence $\cc$, we have  an isomorphism of cones:
\[
Lf^*\colon \BBQ^{\cc}(S)\to \BBQ^{\cc}(R).
\]
\end{cor}
...
%
We remark that $f_*\colon \CQ^0(X,L)\to \CQ^{0}(\PP^n, \cO(1))$  is an isomorphism.  This follows from the central argument in
