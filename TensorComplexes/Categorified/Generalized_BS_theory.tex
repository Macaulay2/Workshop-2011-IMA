...
To extend our results from $S$ to $R$, it will be useful to define a functor
\[
...
\]
given by $\beta(\FF)\mapsto \beta(Lf^*\FF)$.  More generally, if $f$ is flat, then for any codimension sequence $\bb$, the above map is well defined.
...
\end{prop}
\begin{proof}
(1)  Flat case is easy.  General case needs to be filled in.
...
Of course, as maps of vector spaces, $Lf^*$ and $Rf_*$ are both the identity maps.  Hence, we immediately obtain the inclusions:
\[
\BBQ^{\bb}(S)\subseteq \BBQ^{\bb}(R) \quad \text{ and } \CQ^k(X)\subseteq \CQ^{k-d}(\PP^n).
\]
Based on these inclusions, it turns out that $Lf^*$ and $Rf_*$ interact well with the functors $\Phi$ and $\Phi_X$.
