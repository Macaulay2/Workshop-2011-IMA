...
%Here, $\BBQ^{n+1}(S)$ and $\BBQ^{1}(A)$ denote, respectively, the cones of Betti tables of free complexes with homology of  finite length (codimension at least $n+1$ over $S$, or at least $1$ over $A$); and $\CC^{0}(\P^{n})$ denotes the cone of cohomology tables of coherent sheaves on $\P^{n}$. 
\david{should we get rid of the names of the cones and the explanation?}
}
\label{fig:bracket}
\end{figure}

The cone $\BBQ^{1}(A)$ of Betti tables on $A$ with finite length homology (which we denote $\BBQ^1(A)$) is easy to describe, and it is easy to write down the positive functionals that define it.
...
\end{example}
\david{maybe move Example~\ref{ex:res over section ring of ell curve} here?}
\daniel{Alternately, we could change example~\ref{ex:res over section ring of ell curve} to a hyperelliptic curve?}
...
\daniel{I don't know if this example is helpful or not.  It is related to Example~\ref{ex:sup2}, also new.}
...
\begin{example}[Zero dimensional schemes]\label{ex:zero dim}Let $\Gamma=\Spec(B)$ be a $0$-dimensional scheme over $\Spec(\kk)$, where $B$ is an Artinian $\kk$-algebra.  Let $f\colon \Gamma\to \PP^n$ be any morphism and let $R:=B[t]$.  The cone $\BBQ^1(R)$ of positive, free $R$-complexes with finite length homology is the same as the cone $\BBQ^1(A)$ of positive free $A$-complexes with finite length homology.  \david{this is not a good example: the ring $B[t]$ is flat
over $\kk[t]$ with generators in degree 0, so one can simply pull back a complex to get a complex
with the same Bett numbers, or push forward the complex to get one with a multiple of the Betti numbers. This doesn't work in general because the Ulrich module is, in other cases, not a 
(multiple of) the structure sheaf.} 
\end{example}
...
\begin{example}[Elliptic curve]\label{ex:res over section ring of ell curve}
Let $E$ be a smooth elliptic curve with a point $P\in E$, and let $L=\cO_{E}(2P)$ be a degree $2$ line bundle on $E$.   Let $f\colon E\to \PP^1$ be the $2$ to $1$ map induced by $|L|$, and let $R=R(E,L)$ be the section ring.  The ring $R$ is isomorphic to $\kk[x_0,x_1,y]/(g(x_0,x_1,y))$ where $\deg(x_i)=1,\deg(y)=2$, and $\deg(g)=4$. 
