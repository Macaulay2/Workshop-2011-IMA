...
\end{defn}
\david{later we sometimes forget the word ``shifted''. Drop it from the defn, or put it in everywhere!}
...
$$
with $d_{k}k\dots, d_{k+\ell}\in \ZZ$.
...
$$
has $i$-th homology $\HH_{i}\FF$ of codimension $\geq\ell$ for $i>k$, then $\FF$ is actually a (shifted) resolution, and the module $\HH_{k}\FF$ that it resolves must have codimension $\leq \ell$. Thus to say that a pure complex has degree sequence and codimension sequence satisfying the following compatibility
condition is the same as saying that it is a homologically shifted resolution of a module whose codimension is between
$c_{k}$ and $\ell$. This module is Cohen-Macaulay if and only if its codimension is exactly $\ell$.
...
\]
\end{cor}
As noted in the introduction, the denominators of the $a_k$ may be seen as a measurement of the extent to which $\FF$ fails to be quasi-isomorphic to its homology.  

\david{add an example showing how the decomposition proceeds, from higher to lower codim and from right to left.}

\david{put examples from section 7 here}
...
\david{move to a new ``monads'' section that might replace the current section 7 (or come before the proof of the decomp theorem. Explain that this is NOT a decomposition in a chain.}
...
\david{clarify that the first half is about resolutions, so the non-existence comes from the older
B-S theory -- and that the point is to compute the actual complex $\FF\cdot \cE$.}
...
\begin{example}
\david{ use the notation with the $^\circ$ here -- I think the $[-1]$ is then unnecessary.}
...
\david{the next material needs to be cleaned up and made into its own ``monad'' section}
Recall that, for a sheaf $\cF$ on $\PP^n$, we say that $\FF=[\dots \gets \FF_{-1}\gets \FF_0\gets \FF_1\gets \dots]$ is a \defi{free monad} for $\cF$ if $\HH_0\widetilde{\FF}\cong \cF$ and if $\HH_i(\widetilde{\FF})=0$ otherwise.   See \cite[\S8]{eis-floy-schrey}, and the references therein, for more on free monads.  
