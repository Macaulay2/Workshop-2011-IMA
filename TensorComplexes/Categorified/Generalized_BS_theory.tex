...
carries over to the more general situation.) The situation for Betti tables of resolutions over $R$ is not at all analogous to the situation of resolutions over $S$. But we prove a complete analogy for bounded complexes with finite length homology:\daniel{At the moment, there's a $|\kk|=\infty$ assumption needed in the proof of this corollary.  So this needs to be reconciled.}
...
\daniel{The infinite field hypothesis is used to get that $Lf^*$ is well defined.  We could probably prove Corollary~\ref{cor:isom cones} as stated without a $\kk$ infinite hypothesis.}
\begin{cor}\label{cor:new graded rings}
Let $\kk$ be an infinite field, and let $R$ be a graded ring of the form $R=R(X,L)$ where $f\colon X\to \PP^n$ is finite and where $X$ contains an Ulrich sheaf.  and Let $\bb$ be a codimension sequence such that $b_i\leq \dim(R)$ for all $i$.  Then we have  an isomorphism of cones:
\[
Lf^*\colon \BBQ^{\bb}(S)\to \BBQ^{\bb}(R).
\]
\end{cor}
...
Set $d:=\dim(X)=\dim(R)-1$.  Since $b_i\leq d+1$ for all $i$, Theorem~\ref{thm:extremal rays refined} implies that $\BBQ^{\bb}(S)=\BBQ^{\bb}(\kk[x_0,\dots,x_d]).$  Hence, we may replace $f$ by a general linear projection $X\to \PP^d$, thus replacing $S$ by $\kk[x_0,\dots,x_d]$, and it suffices to prove the theorem in this case.
