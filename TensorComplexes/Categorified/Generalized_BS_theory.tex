...
The theory has been further extended by~\cites{boij-sod2,eis-schrey2}, using the original functionals $\langle -,-\rangle_{\tau,\kappa}$.
But the nature of these functionals has remained mysterious.  

In this paper, we present a  robust foundation for the duality between syzygetic and cohomological invariants that categorifies
and, in some sense, explains the Eisenbud-Schreyer functionals. 
Using the new foundation we  extend the reach of the theory to complexes with homology and to many rings other than polynomial rings, where the only previous progress was that of~\cite{bbeg}. We also present a framework for an extension to the multigraded case of toric rings, where the only previous progress was that of~\cites{boij-floystad,floystad-multigraded}.
...
we construct a single pairing that unifies and categorifies all of the functionals $\langle-,-\rangle_{\tau,\kappa}$. The pairing takes values in a derived category of graded modules over a polynomial ring in 1 variable and gives a new foundation for \BS theory.
...
(or equivalently, the consideration of elements of the derived category).\daniel{What do you think of this parenthetical remark?}
