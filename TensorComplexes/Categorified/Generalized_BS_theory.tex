...
\david{I removed the definition of multigraded betti and cohomology---seemed too obvious.}

%For
%$i\in \ZZ$, and $\alpha\in \Pic(X)$
%we define the multigraded Betti numbers to be 
%$\beta_{i,\alpha} \FF:=\dim \Tor_i(\FF,\kk)_{\alpha}$ and we define the multigraded cohomology numbers to be $\gamma_{i,\alpha} \cE:=\dim H^i(X, \cE(\alpha))$.
...
The Cox ring $R$ of $X$ comes equipped with a natural irrelevant ideal. We say that a complex $\FF$ over $R$ or $C$ has \defi{irrelevant homology} if its homology is supported on the irrelevant ideal.  (See Definition~\ref{defn:irrC} for the irrelevant ideal of $C$).
...
\david{Why should these definitions be numbered? I would like to get rid of the definition
environment, which seems a bit arbitrary unless theres a reference to it.}

\begin{defn} A \defi{codimension sequence} is an indexed, nondecreasing,
...
field extensions do not change the cones of rational Betti tables (we do not know whether they change the semigroups of Betti tables themselves.) \david{Is this remark ok with you, Daniel?}
...
$\CQ^0(X,L)$ and $\CQ^{0}(\PP^n)$ are the same;
 although $L$ was assumed to be very ample in that proof, the  argument works perfectly just assuming
 that $f$ is finite.
 
 Next we wish to apply duality. The proof of Theorem~\ref{thm:extremal rays refined} in the case of $\BBQ^{\cc}(S)$ holds for $\BBQ^{\cc}(R)$ as well.  We provide a sketch of this argument.  Much of the proof of Theorem~\ref{thm:extremal rays refined} boils down to an analysis of degree sequences and pure Betti tables that are compatible with $\cc$, and these arguments go through unchanged.  The other key inputs for the proof of Theorem~\ref{thm:extremal rays refined} are the positivity results derived from Theorem~\ref{thm:Phi} and Lemma~\ref{lem:refined positivity}.  Theorem~\ref{thm:PhiX} plays the role of Theorem~\ref{thm:Phi}, and we can mimic the proof of Lemma~\ref{lem:refined positivity} to obtain an analogue for $R$-complexes that are compatible with $\cc$.  

Putting these two results together, we see that $\BBQ^{\cc}(S)=\BBQ^{\cc}(R)$ as required. \end{proof}

\david{a version of the old proof is commented out below. It seems to me repetitious. Am I missing something?}
%By Lemma~\ref{lem:ground field}, we may assume that $\kk$ is infinite. Suppose that $\FF$ is a bounded
...
%$
%
%On the other hand, if $\cE$ is a bounded complex of sheaves on $X$ then since $f$ is finite, $f_{*}$ commutes
%with cohomology, so $\CQ^k(X,L)\subseteq \CQ^{k-d}(\PP^n)$. 
%In fact, because of the existence of an Ulrich sheaf for $f$, the map $f_*\colon \CQ^0(X,L)\to \CQ^{0}(\PP^n)$  is an isomorphism.  This follows from the central argument in
...
Let $R$ satisfy the hypotheses of Theorem~\ref{cor:new graded rings} and let $\FF$ be a (possibly infinite) minimal free resolution of a finite length, graded $R$-module.  Then we may write the Betti table of $\FF$ as a positive, rational sum of (possibly infinitely many) shifted resolutions of finite length modules with finite projective dimension.  I.e., we may write:
\[
...
\]
Since $\FF$ resolves a finite length module, it follows that $\Hom(\FF_{\leq e},R)$ is compatible with the codimension sequence $\cc=(\dots,0,0,0^*,n+1,\dots, n+1^\zp, n+1,\dots)$, where the $0^*$ lies in homological degree $-e$.  By Theorem~\ref{cor:new graded rings}, we can decompose $\beta(\Hom(\FF_{\leq e},R))$ with pure Betti tables whose degree sequences are compatible with $\cc$ and we can thus write:
\[
