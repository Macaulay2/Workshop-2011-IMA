...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Categorified Eisenbud--Schreyer Positivity}\label{sec:functionals}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\right\},
\]
where the shifted degree sequence $d$ corresponds to the ray spanned by the shifted pure resolution of type $d$ of a Cohen--Macaulay module. Further, the cone $\BBQ^{k}(S)$ has a simplicial fan structure, where the simplices correspond to chains of shifted degree sequences.
\end{cor}
...
Let $\bb=(\dots,\emptyset,0^\zp,n-1,n-1,\dots,)$.   Let $\FF\in \DD^b(S)$ be compatible with $\bb$ and let $M$ be the $0$th homology module of $\FF$.  We note that $\FF$ is like an approximate resolution of $M$.  Such approximate resolutions play a key role in \cite[Lemma~1.6]{gruson-lazarsfeld-peskine}, where it is that the Castelnuovo--Mumford regularity can be bounded by approximate resolutions.
\end{example}
...
\end{lemma}
\begin{proof}
We first consider the nondecreasing case.  Let $\FF$ be compatible with $\bb$ and let $\cE$ be a coherent sheaf of codimension $\geq n+1-k$.  By replacing $\cE$ by a general $\GL_{n+1}$ translate, we may assume that $\FF$ and $\cE$ are homologically transverse~\cite{miller-speyer}.  \daniel{At the moment there's no official definition of homological transversality, so we should handle it in the course of the proof.} It then follows that $\widetilde{\FF}\otimes \cE$ is exact in homological degrees $\geq 0$. \david{give the argument: a spectral sequence with $E^{2}$ term $\Tor(\cE, H_{*} \FF)$ converging
...
To begin with, we have a well defined map:
\[
f^*\colon \BBQ^{\bb}(S)\to \BBQ^{\bb}(R)
\]
defined by sending $\beta^S(\FF)\mapsto \beta^R(f^*\FF)$.  Of course, for $f^*\FF$ to be well defined on the quasi-isomorphism class of $\FF$, we need to represent $\FF$ by a free complex.  For a free complex $\FF$, we remark that if $\FF$ is compatible with $\bb$ then $f^*\FF$ is also compatible with $\bb$.\daniel{Is this really true?  We had claimed it was true over the phone, but now I doubt it.  What if $f$ is the blowup of $\PP^2$ at the origin and $\FF$ is the minimal resolution of the origin.  Won't $\FF$ have codim $2$ whereas $f^*\FF$ will have codim $1$?  Do we need a sort of transversality argument again?  Then we can't pullback something arbitrary\dots}

We also have a well defined map on cohomology tables:
\[
Rf_*\colon \CQ^k(X)\to \CQ^{k-d}(\PP^n).
\]
where $\gamma(\cE)\mapsto \gamma(Rf_* \cE)$.  The fact that this is well defined on cohomology tables follows from the projection formula combined with\daniel{need to fill this in.}


Of course, as maps of vector spaces, $f^*$ and $Rf_*$ are both the identity maps.  Hence, we immediately obtain the inclusions:
\[
\BBQ^{\bb}(S)\subseteq \BBQ^{\bb}(R) \quad \text{ and } \CQ^k(X)\subseteq \CQ^{k-d}(\PP^n).
\]
Based on these inclusions, it turns out that $f^*$ and $Rf_*$ interact well with the functors $\Phi$ and $\Phi_X$.

\begin{lemma}\label{lem:general pairing}
We have the equality:
\[
\beta(f^*\FF\cdot \cE)=\beta(\FF\cdot Rf_*\cE).
\]
\end{lemma}
\begin{proof}
...
It is natural to ask when $f^*$ and $Rf_*$ induce isomoprhisms of cones.  A partial answer is given by Corollary~\ref{cor:isom cones}.
%For this, we recall that $\cU$ is an Ulrich sheaf for $(X,L)$ if $f_*(\cU)\cong \cO_{\PP^d}^r$ for some $r>0$.  See~\cite{XXXX} for more on Ulrich sheaves.



\begin{proof}[Proof of Corollary~\ref{cor:isom cones}]
\daniel{Needs to be rewritten somewhat.  Also, we need to extend to $\BBQ^{\bb}$ if we're going to do that.}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Toric/Multigraded generalizations}\label{subsec:toric}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Infinite resolutions}\label{subsec:infinite}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
