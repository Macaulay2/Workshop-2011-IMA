...
\begin{abstract} Let $\kk$ be a field and let $S = \kk[x_{0}, \dots,x_{n}]$ be the homogeneous coordinate ring of $\PP^{n}_{\kk}$.
...
In addition, it has been quite difficult to extend the theory to resolutions over other rings, and only a little progress has been made~\cites{beks-local,boij-floystad,eis-schrey-abel,floystad-multigraded}. Though it seemed natural to consider the multi-graded case through toric varieties, there has been essentially no progress in this direction.

In this paper we interpret all of the functionals $\langle-,-\rangle_{\tau,\kappa}$ in terms of a single a pairing on derived categories. The key surprise is that this pairing takes values in a derived category over $A=\kk[t]$, the polynomial ring in 1 variable.  This categorification allows us to extend the theory in several directions.  First, we extend the theory from free resolutions to the case of finite free complexes with homology. Second,  working with more general complexes provides a more robust theory which then extends naturally to many other graded rings.  Third, our pairing admits a simple generalization to the multigraded case.
...
Following a common convention we display the Betti table of $\FF$ as a table
...
carries over to the more general situation.) The situation for Betti tables of resolutions over $R$ is not at all analogous to the situation of resolutions over $S$, but it is analogous for bounded complexes with finite length homology:
...
This corollary provides the first descriptions of cones of Betti tables over a ring $R$ with a nonstandard grading, as in the following example.
...
$\kk[x_1,x_2,z]/(z^{2}-g(x_{1},x_{2}))$  where $\deg(x_i)=1$ and $\deg(z)=2$. \daniel{The old version of this example had a serious typo, I think.  Does
...
\begin{notation}
As with Betti tables, we use a $\zp$ to indicate homological position zero when writing a degree sequence.  For example, we write $d=(\dots, -\infty, 0^\zp,1,2,\infty, \dots)$ for the degree sequence $d_{-1}=-\infty, d_0=0,$ and so on.
\end{notation}


Given any degree sequence $d$, we say that a complex $\FF$ is \defi{pure of type $d$} if, for all $i$ such that $d_i\in \ZZ$, the free module $\FF_i$ is generated entirely in degree $d_i$. If $P$ is some property that may be applied to a graded $S$-module, we extend the definition to the derived category by saying that $\FF \in \DD^b(S)$ has property $P$ if the direct sum of the homology modules of $\FF$ have this property. The existence of pure resolutions (see~\cite{efw} or \cite[\S5]{eis-schrey1}) shows that, for any shifted degree sequence $d$ of codimension $\ell$, there exists a pure, Cohen-Macaulay complex $\FF$ of type $d$ and codimension $\ell$ (which is automatically a resolution).  
...
We similarly take the definition of any property that may be applied to a coherent sheaf, and we extend this to any $\cE\in \DD^b(\PP^n)$.  \daniel{do we need root sequence definitions here?}

\begin{defn}
We index complexes in $\DD^b(S)$ homologically as in $\FF=[\dots \gets \FF_0\gets \FF_1\gets \dots]$.  For any $k\in \ZZ$, we define a \defi{homological shift} of $\FF$, denoted $\FF[k]$, as the complex obtained by shifting the indices in the following way:  $(\FF[k])_i=\FF_{i-k}$.\daniel{Is this correct?  This really looks a ``cohomological shift'', but I think this is what we discussed.}
\end{defn}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
In this section, we introduce a refined version of Boij--S\"oderberg theory that enables the numerical study of complexes $\FF$ where the homology satisfies different bounds in different homological positions.  Our main result is Theorem~\ref{thm:extremal rays refined} which provides a wide array of refined analogues of the main decomposition results in Boij--S\"oderberg, including those of \cites{eis-schrey1,boij-sod2} (see Example~\ref{ex:resolutions} below).  

Our proof of Theorem~\ref{thm:extremal rays refined} involves two essential steps.  First, in \S\ref{sec:A}, we provide a detailed description of cones of Betti tables on $A$.  Since $\DD^b(A)$ is the target of the pairing $\Phi$, these results will provide a base case for our main results.  Second, in \S\ref{sec:proof refined}, 
...
\begin{defn}
Let $\bb=(\dots, b_i, b_{i+1}, \dots ) \in \bigoplus_{\ZZ} \{0,1 \dots, n+1,\infty\}\cup \{\emptyset\}$ be a nondecreasing or nonincreasing sequence, where we take the convention that $\emptyset<0$.   Let $\FF\in \DD^b(S)$ be represented by a minimal, free complex.  We then say that a$\FF$ is \defi{compatible with $\bb$} if 
\[
...
We define $\BBQ^{\bb}(S)$ as the positive subcone of $\VV$ spanned by $\beta(\FF)$ as $\FF$ ranges over all elements of $\DD^b(S)$ that are compatible with $\bb$.  We use the notation $\BBQ^k(S)$ for the special case $\bb_i=k$ for all $i$.
\end{defn}

It will be useful to define several cones of cohomology tables.
\begin{defn}
For any $k=0, \dots, n$, we define $\CQ^k(\PP^n)$ to be the subcone of $\WW$ spanned by all $\cE\in \DD^b(\PP^n)$ satisfying $\codim(\cE)\geq k$.
\end{defn}

Our goal is to describe a simplicial fan structure of $\BBQ^{\bb}(S)$ in terms of pure Betti tables.  This leads to the following definition.

\begin{defn}\label{defn:deg compatible}
Fix a nondecreasing (respectively nonincreasing) sequence $\bb$ and a degree sequence $d$ of codimension $\ell$.  Recall that $\FF_d$ denotes a pure complex of type $d$ and codimension $\ell$, and let $k$ be the unique homological position where $\FF_d$ has homology.  We say that $d$ is \defi{compatible with $\bb$} if $\bb_k\leq \ell\leq \bb_{k+1}$  (respectively, if $\bb_k\geq \ell\geq \bb_{k+1}$).
\end{defn}

\subsection{Summary of results}
Our main result about decomposing Betti tables is the following.
...
This leads to a wide generalization of Corollaries~\ref{cor:decompose} and \ref{cor:decompose2}.
...
Let $\bb$ be the nonincreasing sequence $\bb=(\dots, n+1, \dots, n+1, 0^\zp,-\infty, \dots)$.  For any sheaf $\cE$ on $\PP^n$, let $\FF=[\FF_{-p}\gets \dots \gets \FF_0]$ be a positive, free complex such that 
\[
...
%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%
We first prove the special case of Theorem~\ref{thm:extremal rays refined} given by considering complexes of $A$-modules whose homology modules have finite length.  This leads to a description of $\BBQ^1(A)$.  We treat this case separately to help clarify our discussion in \S\ref{sec:functionals and duality}, where will interpret the functionals $\langle -,-\rangle_{\tau,\kappa}$ in terms of $\Phi$ and where we will discuss the duality of cones induced by $\Phi$.
...
The key to proving Theorem~\ref{thm:extremal rays refined} is to combine Theorem~\ref{thm:Phi} with Theorem~
...
\daniel{This proof needs to be updated and cleaned up.}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
In this section we compute a few examples to help clarify the results from the previous sections.
...
Note that the nonnegativity of all such functionals  follows by applying Theorem~\ref{thm:Phi}\eqref{thm:Phi:2} and Proposition~\ref{cor:dualconeA}.
A similar statement holds for each nonnegative functional on cohomology tables.  Thus, the functor $\Phi$ provides a unifying view on all of the Eisenbud--Schreyer functionals and on the corresponding positivity results, including~\cite[Positivity 1]{eis-schrey-icm} and~ \cite[Positivity 2]{eis-schrey-icm}.
...
\daniel{This section will disappear.  But right now there are lots of hanging references to the proof of the big theorem.}
