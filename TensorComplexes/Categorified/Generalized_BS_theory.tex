...
The Hilbert polynomial is a fundamental invariant of graded modules or coherent sheaves on projective space. This invariant is refined in two different ways by the Betti table of a graded module and the cohomology table of a coherent sheaf. Work of Eisenbud and Schreyer \cite{eis-schrey1} suggests a sort of  duality between these extensions, involving many numerical functionals. In this paper we clarify the duality, showing that it is embodied in
a single pairing between derived categories.

 Eisenbud and Schreyer applied the their theory to prove the \BS conjectures~\cite{boij-sod1},
which describe, up to scalar multiple, the possible values of certain invariants coming from finite free resolutions over a polynomial ring.  
...
Using our categorification we  extend the reach of the theory to complexes with homology and to many rings other than polynomial rings, where the only previous progress was that of~\cite{bbeg}. We also present a framework for an extension to the multigraded case of toric varieties, where the only previous progress was that of~\cites{boij-floystad,floystad-multigraded}.
