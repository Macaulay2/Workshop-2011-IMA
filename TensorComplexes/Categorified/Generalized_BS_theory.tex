...
Suppose that $R$ satisfies the hypotheses of Theorem~\ref{cor:new graded rings}. If $\FF$ is a (possibly infinite) minimal free resolution of a finite length, graded $R$-module, then  
\[
\beta(\FF)=\sum_{k=0}^\infty a_k\beta(M_k)[k],
\]
with $a_k\in \QQ_{\geq 0}$, where each $M_k$ is an $R$-module of finite length and finite projective dimension.
\end{cor}
\begin{proof}
Let $n=\dim(R)-1$.  For $e\in \ZZ$, we set
\[
...
$\beta(\Hom(\FF_{\leq e},R))$ as a positive rational linear combination of pure Betti tables whose degree sequences are compatible with $\cc$, say
\[
\beta(\Hom(\FF_{\leq e},R))=\left(\sum_{k=-e+1}^{-n-1} a_k\beta(N_k)[k]\right)+\beta(N')[-e],
\]
with $a_k\in \QQ_{\geq 0}$, where each $N_k$ is a finite length module of finite projective dimension, and  $N'$ has finite projective dimension.  It follows from the form of the algorithm that, as long as $k>-e$, the $a_k$ and  $\beta(N_k)$ do not depend on $e$.  Thus, as $e\to \infty$,  the decomposition  stabilizes to an infinite sum
\[
\beta(\Hom(\FF),R)=\sum_{k=-\infty}^{-n-1} a_k\beta(N_k)[k],
\]
where each $N_k$ a finite length module.  By Peskine and 
Szpiro's ``Lemme d'Acyclicit\'e'' \cite{}, the duals of the resolutions of the $N_{k}$
are finite resolutions of the modules $M_{k}:=\Ext_{R}^{\text{depth }R}(N_{k}, R)$ so we can use them to get the desired decomposition.
\david{I changed this from the resolutions of $Hom_{\kk}(M_{k}, \kk)$, which is different
unless $R$ is Gorenstein and you put in a twist.} 
\end{proof}
%
%\begin{example}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
In this section, we extend the pairing $\Phi$ to a toric variety and prove Theorem~\ref{thm:Phimulti}.  We use this pairing to construct multigraded analogues of the Eisenbud--Schreyer functionals.  With $\chi_{i,\alpha}$ and $\Phi_X$ as defined below, we will prove the following nonnegativity result:
