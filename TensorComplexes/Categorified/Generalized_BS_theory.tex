...
\begin{defn}
We say that $\FF\in \DD^b(S)$ \defi{ is homologically transverse to $R$} if we have:\daniel{redo}
\begin{itemize}
	\item $\Tor^S_q(\HH_r(\FF),R)=0 \text{ for } q>0$, and all $r$, and
	\item $\codim \HH_r(\FF)\otimes_S R= \min\{d+1, \codim \HH_r(\widetilde{\FF})+\codim \cE\}.$
\end{itemize}
\end{defn}

\daniel{Infinite field???}
\begin{prop}\label{prop:pushpull}
Let $(X,L)$ and $f$ as above, and assume that $\kk$ is infinite.
\begin{enumerate}
	\item Let $\bb$ be any codimension sequence.   Then we have a well defined map of cones
	\[
...
\end{cor}
\begin{proof}
Extending the ground field $\kk$ has no effect on $\BBQ^{\bb}(S)$.  Since such an extension is flat, it can only potentially increase the size of $\BBQ^{\bb}(R)$.  Since we already have shown that $Lf^*$ is an inclusion of cones, it suffices to prove the corollary after passing to an infinite ground field $\kk$.  Since $b_i\in \{\emptyset, 0,d+1\}$ for all $i$, Theorem~\ref{thm:extremal rays refined} implies that $\BBQ^{\bb}(S)=\BBQ^{\bb}(\kk[x_0,\dots,x_d]).$  Hence, we may replace $f$ by a general linear projection $X\to \PP^d$, thus replacing $S$ by $\kk[x_0,\dots,x_d]$, and it suffices to prove the theorem in this case.

We first remark that $Rf_*\colon \CQ^0(X,L)\to \CQ^{0}(\PP^n, \cO(1))$  is an isomorphism.  Since $f$ is finite, we obtain the inclusion $\CQ^0(X,L)\subseteq \CQ^0(\PP^n,\cO(1))$.  Now, let $\cE$ be a coherent sheaf on $\PP^n$, and we seek to show that $\gamma(\cE)$ lies in $\CQ^0(X,L)$.  Let $\cU$ be an Ulrich sheaf for $(X,L)$.  The central computation in \cite[Proof of Theorem 5]{eis-schrey-abel} shows that the cohomology table of $\cU\otimes f^*\cE$ is a scalar multiple of the cohomology table of $\cE$. This proves that $\CQ^0(X,L)= \CQ^{0}(\PP^n,\cO(1))$ as desired.
...
%It follows by a spectral sequence computation that 
%the complex $R\pi_{2*}\GG$ on $\Spec A[t,t^{-1}]$ has no homology. By flat base change,
%this is equal to the restriction of $\Phi(\FF,\cE)$ on the open set $t^{-1}$, and we see that the homology
%of $\Phi(\FF,\cE)$ is annihilated by a power of $t$, as required.
\end{proof}
