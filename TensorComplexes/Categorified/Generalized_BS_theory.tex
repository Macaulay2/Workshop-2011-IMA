...
If  $\FF$ is a complex with irrelevant homology and if $\cE$ is a vector bundle, then $\widetilde{\FF}\otimes \cE$ is always exact.
...
No multiple of the Betti table $\beta(\FF)$ cannot equal the Betti table of a minimal, free complex with finite length homology. Indeed, such a complex would be a resolution,  so this case is covered by the theory of \cite{eis-schrey1}. But it is instructive to see how this follows directly from Theorem~\ref{thm:Phi}.

In fact, if $\cE$ to be a rank $8$ supernatural bundle of type $(0,-8)$ then 
...
\begin{remark}
We refer throughout to the definition of $\langle -, -\rangle_{\tau,\kappa}$ given in \cite{eis-schrey-icm}.  Though the same functionalsappear in \cite{eis-schrey1}, they are indexed differently.
\end{remark}
...
the case when $\cE$ is supernatural, we may use \cite{eis-schrey2} to extend this linearly to the case of an arbitrary $\cE$.)
...
\end{bmatrix}.
\]
The boldfaced entries indicate the shape of minimal pure diagram that is compatible with $\cc$ and that could contribute to $\beta(\FF)$, and this corresponds to the degree sequence $(\dots,\emptyset^\zp,2,3,4,6,\infty,\dots)$. We apply a greedy algorithm, subtracting as much of the corresponding pure diagram from $\beta(\FF)$ as possible, without making any entry negative.  This yields:
\[
\begin{bmatrix}
1&-&-&-&-\\
-&\mathbf{5}&\mathbf{4}&\mathbf{1}&-\\
...
\end{align*}
Note that, in the third step of the decomposition, we move from a codimension $3$ degree sequence to a codimension $2$ degree sequence.  This is because the degree sequence $(\dots, 0^\zp,2,3,5,\infty,\dots)$ is not compatible with $\cc$.
\end{example}
...
\daniel{The discussion in \S3 changed a bit, so we might need to rephrase the above paragraph slightly.}
...
If $c_s=0$, then $\beta(A(-t)[-s])$ is compatible with $\cc$, and we can set $u'=u-\beta(A(-t)[-s])$.  On the other hand, if $c_s>0$, then we choose $r$ so that $u_{s-1,r}$ is the top nonzero entry in column $s-1$.  We claim that, $r<t$, so that $u$ has the form:
\[
u=
\left[
\begin{array}{ccccccc}
...
\end{itemize}
In fact, since $\FF$ is bounded, a general translate of $\cE$ will be homologically transverse to all of the $\HH_r(\widetilde{\FF})$ simultaneously.  Further, based on the second condition and the fact that $c_0=k$, we have $(\HH_r\widetilde{\FF})\otimes \cE=0$ whenever $r\geq 0$.
...
We observe that, for any maximal chain in $P_{(\delta,\epsilon)}$, the corresponding pure Betti tables span the codimension $c_{\delta}$ subspace of $\VV_{(\delta,\epsilon)}$ cut out by the vanishing of the first $c_{\delta}$ Herzog--K\"uhl equations.
...
\[
\FF_d\cdot \cE_f=\left[{A^N}(-\epsilon)\gets A^N(-\delta-b) \right],
\]
...
Let $\FF$ be a free monad for a sheaf $\cF$ on $\PP^n$.  Then there exists: a free complex $\FF'=[\FF'_0\gets \FF'_1\gets \dots]$ such that $\widetilde{\FF}'$ resolves a coherent sheaf of $\PP^n$ of rank equal to the rank of $\cF$; a free complex $\FF''=[\FF''_0\gets \FF''_1\gets \dots]$ such that $\widetilde{\FF}''$ resolves a coherent sheaf of rank $0$ on $\PP^n$; and positive rational scalars $\lambda', \lambda''$, such that
\[
...
\begin{proof}
Throughout this proof, we take the convention that, for any degree sequence $d$, $\pi_d$ denotes the Betti table of some shifted pure resolution of type $d$.

Note that $\beta(\FF)$ is compatible with the codimension sequence $\cc'=(\dots,0,0,0^{\zp},n+1,\dots)$. We may thus decompose $\beta(\FF)$ according the decomposition algorithm induced by the simplicial structure on $\BBQ^{\cc'}(S)$.  There then exists a collection of nonnegative $a_i\in \QQ_{\geq 0}$ and a chain of degree sequences $d^0<d^1<\dots <d^s$, such that each $d^i$ is compatible with $\cc'$ and such that
\[
\beta(\FF)=\sum_{i=0}^s a_i\pi_{d^i}.
\]
Since the $d^i$ form a chain, there is some maximal $r$ such that the degree sequence $d^r$ has codimension $>0$.  We define $D':=\sum_{i=0}^r a_i\pi_{d^i}$.  We note that, for $i\ne 0$, we can describe the Betti numbers $\beta_{i,j}D'$ explicitly:
\[
...
We next consider the dual Betti table $\beta(\Hom(\FF,S))$.  Note first that $\Hom(\FF,S)$ is compatible with the codimension sequence $\cc''=(\dots,0,0,0^{\zp},n+1,n+1\dots)$.  As above, we obtain a decomposition $\beta(\Hom(\FF,S))=\sum_{i=0}^{s'} b_i\pi_{e^i}$ according to the decomposition algorithm induced by $\BBQ^{\cc''}(S)$.  We define $D''$ to be the sum $D''=\sum_{i=0}^{t} b_i\pi_{e^i}$ where $t$ is the maximal value such that $e^t$ the degree sequence has codimension $>0$.  
...
We next claim that all entries of $E$ are nonnegative.  This follows from the method by which the decomposition algorithm proceeds.  Namely, within a single column, the decomposition algorithm works from top to bottom.  So $D'$ consists of the lowest degree syzygies from column $0$.  Since dualizing inverts degrees, $(D'')\vee$ consists the highest degree syzygies from column $0$.  Thus, the only possible problem would be if $D'$ and $(D'')^\vee$ overlapped in column $0$, and in this case, all entries of $E$ would be strictly negative, contradicting the computation in the previous paragraph.

We now conclude the proof.  By construction, we may choose a scalar $\lambda'$ and a free complex $\FF'$ that is compatible with $\cc'$, such that $\lambda'\beta(\FF')=D'+E$.  Since $\FF'=[\FF'_0\gets \dots]$ and is compatible with $\cc'$, it follows that $\widetilde{\FF}'$ resolves a coherent sheaf $\cF'$ on $\PP^n$.  Further the rank of $\cF'$ equals $\sum_{j} \beta_{0,j} E$ which equals the rank of $\cF$.  Also by construction, we may choose a scalar $\lambda''$ and a free complex $\FF''=[\FF''_0\gets \dots]$ that is compatible with $\cc''$, such that $\lambda''\beta(\FF'')=D''$.  It also follows that $\widetilde{\FF}''$ resolves a rank $0$ sheaf $\cF''$.
\end{proof}
...
It now suffices to handle the case of the purely transcendental extension $K=\kk(z)$.  Let $\FF\in \DD^b(R\otimes_{\kk}K)$ be a free complex that is compatible with $\cc$.  By clearing denominators, we may extend $\FF$ to a complex over $\kk[z][x_0,\dots,x_n]$.  The locus of $\Spec(\kk[z])$ where $\FF$ fails to be compatible with $\cc$ will be a closed subset of $\Spec(\kk[z])$.  Since $\FF$ is compatible with $\cc$ over the generic point, it follows that follows that we can choose a closed point $P\in \Spec(\kk[z])$ such that the restriction of $\FF$ to $P$ is still compatible with $\cc$.  Since the residue field of $P$ is finite over $\kk$, we are reduced to the case of a finite extension, and it follows from the above argument that $\beta(\FF)$ lies in  $\BBQ^{\cc}(R)$.
\end{proof}
