...
%takes a bounded complex of
%initely generated graded $S$-modules,  and a bounded complex of coherent sheaves on $\P^{n}$, and produces a bounded complex of finitely generated %graded modules over a polynomial ring in 1 variable. 
%This pairing 
simultaneously categorifies all the functionals used by Eisenbud an Schreyer in their proof of the Boij-S\"oderberg conjectures.
%Based on positivity properties of the pairing, we extend the Eisenbud--Schreyer theorem from free resolutions of $S$-modules to bounded free complexes of $S$-modules with homology modules of specified dimensions
...
It has also been difficult to extend the theory to resolutions over other rings.  For instance, only modest progress has been made for other graded rings~\cite{bbeg}; and although it seems natural to consider the multi-graded case through toric varieties, there has been little progress in this direction~\cites{boij-floystad,floystad-multigraded}.
...
\end{matrix}\right\}
\end{equation*}
As we discuss in \S\ref{sec:toric}, the target of this pairing, i.e.\ the cone of free $C$-complexes with irrelevant homology, is simpler than the cone of free resolutions of over $C$.  This enables the construction of toric/multigraded analogues of the Eisenbud--Schreyer fucntionals, and it thus seems that the cone of free $R$-complexes with irrelevant homology will be easier to study than the cone of free resolutions over $R$. 

\subsection*{Structure of this paper}
\daniel{The last few paragraphs on p.2, as well of the table of contents, both kind of do the same thing as this paragraph.  I propose just dropping it.}
%
...
$$
for each $i$ and where we take the convention $\emptyset<0$.  
...
Suppose that $\bb$ is a codimension sequence  and ${\bf d}$ is a degree sequence  of codimension $\ell$. Let $k$ be the smallest index
...
%such that $d_{k}\in \ZZ$. If $k$ is a nondecreasing index of $\bb$ (resp.\ a nonincreasing index of $\bb$), then we say that $\dd$ is \defi{compatible with $\bb$} if $\bb_k\leq \ell\leq \bb_{k+1}$  (resp.\ if $\bb_{k+\ell}\leq \ell\leq \bb_{k+\ell-1}$).
%\end{defn}
...
%
%We postpone the details until the next section, though we remark that this basic outline simplifies the structure of Eisenbud--Schereyer's proof of the original Boij--S\"oderberg conjectures.  Namely, in~\cite[Proof of Theorem 0.2]{eis-schrey1}, the key technical tool is the construction of nonnegative functionals on the cone of Betti tables.  
%
...
\daniel{I'm not sure the following Corollary needs to be here.  But I just can't shake the feeling that $\BBQ^{n+1}(S)$ is a  distinguished special case, and it doesn't show up much.}
...
%
If $\bb=(\dots, \emptyset, 0^\zp, \infty, \dots)$ then a complex $\FF$ is compatible with $\bb$ if and only if $\FF$ is the minimal free resolution of an $S$-module.  Hence, for this value of $\bb$, Theorem~\ref{thm:extremal rays refined} recovers the main results of \cite{boij-sod2}.  If instead $\bb=(\dots, \emptyset, n+1^\zp, \infty, \dots)$, then we recover \cite[Theorem~0.2]{eis-schrey1}.
\end{example}


\begin{example}[$\cO_{\PP^n}$-resolutions]\label{ex:sheaf resolutions}
If $\bb=(\dots,\emptyset,0^{\zp},n+1,n+1,\dots)$ then a complex $\FF$ is compatible with $\bb$ if and only if $\widetilde{\FF}$ resolves a coherent $\cO_{\PP^n}$-module.
\end{example}

%\begin{example}[Coresolutions]\label{ex:coresolutions}
%We say that $\FF$ is a Let $\bb$ be the nonincreasing sequence $\bb=(\dots, n+1, \dots, n+1, 0^\zp,\emptyset, \dots)$.  For any sheaf $\cE$ on $\PP^n$, let $\FF=[\FF_{-p}\gets \dots \gets \FF_0]$ be a positive, free complex such that 
%\[
...
%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%
Since the output of $\Phi$ is a complex on $A$, our proof of Theorem~\ref{thm:extremal rays refined}  involves a study of cones of Betti tables over $A$. We thus first prove Theorem~\ref{thm:extremal rays refined} for cones of the form $\BBQ^{\bb}(A)$ (which is $n=0$ case of the theorem), as these provide a base case for our main result.  In addition, we provide a halfspace description of each such cone.
...
Here is the general statement for an arbitrary codimension sequence $\bb$.
...
We begin with a lemma which is like a refined version of Theorem~\ref{thm:Phi}\eqref{thm:Phi:2}.  Combining this lemma with Corollary~\ref{cor:dualconeA refined}, we produce lots of nonnegative functionals on $\BBQ^{\bb}(S)$.
...
We proceed by restricting to finite dimensional subcones.  For $(\delta,\epsilon)\in \ZZ^2$ with $\epsilon-\delta\geq n+1$, we set $\VV_{(\delta,\epsilon)}$ to be the subspace of $\VV$ defined by
\[
\VV_{(\delta,\epsilon)}:=\bigoplus_{i=\delta}^\epsilon \bigoplus_{j=-\epsilon +i}^{-\delta+i} \QQ.
\]
We let $P_{(\delta,\epsilon)}$ be the poset of degree sequences that are compatible with $\bb$ and whose corresponding rays lie in $\VV_{(\delta,\epsilon)}$, and we set $\widetilde{\bb}=(b_{\delta},b_{\delta+1},\dots, b_{\epsilon})$ be the truncation of $\bb$.  Since we can ignore any column $i$ with $b_i=\emptyset$, we may, without loss of generality, assume that $b_\delta \geq 0$.  By possibly shrinking $n$ (which will not affect the rest of the proof), we may assume that the minimal degree sequence in $P_{(\delta,\epsilon)}$ has codimension $n+1$.  We may then assume, without loss of generality that $b_{\epsilon-n-1}<\infty$, else we could replace $\epsilon$ by $\epsilon-1$.
...
Let $\Sigma_{(\delta,\epsilon)}$ be the cone spanned by the pure diagrams of type $\dd$, as $\dd$ ranges over the poset $P_{(\delta,\epsilon)}$.  We may apply the proof of  \cite[Proposition~2.9]{boij-sod1} to conclude that $\Sigma_{(\delta,\epsilon)}$ has the structure of a simplicial fan, with simplicies corresponding to chains in $P_{(\delta,\epsilon)}$.

We observe that, for any maximal chain in $P_{(\delta,\epsilon)}$, the corresponding pure Betti tables span the codimension $b_{\epsilon-n-1}$ subspace of $\VV_{(\delta,\epsilon)}$ cut out by the vanishing of the first $b_{\epsilon-n-1}$ Herzog--K\"uhl equations.
...
We now come to a significant difference between this proof and similar proofs from \cites{eis-schrey1,boij-sod2}.  There are three ways that  adjacent elements $d<d'$ can arise in $P_{(\delta,\epsilon)}$.   The first possibility is simply that $d'$ is obtained from $d$ by adding $1$ to a single entry, as in:
\[
(\dots,3^\circ,\dots)<(\dots,4^\circ,\dots).
\]

The second possibility is when the finite entries of $d$ and $d'$ lie in different homological positions, and this can only arise in a specified way.  Namely, let $c$ be the codimension of $d$.  For this case to occur, there exists a unique column $i$ such that: $d_j=d'_j$ for $j\notin \{i, i-c-1\}$; $d_i=i+e$ and $d'_i=\infty$; and $d_{i-c-1}=\emptyset$ and $d'_{i-c-1}=i-c-1-e$.  For example, if $c=2$ and $i=0$, we could have:
\[
...
%(Note that, since $\bb$ is nondecreasing, we only ever face codimension shifts which increase the codimension.  When $\bb$ is nonincreasing, we will only ever face codimension shifts which decrease the codimension.)\daniel{This sentence was where we needed more details in the nonincreasing case.}
...
As in, e.g. \cite[Proposition~2.12]{boij-sod1}, these halfspaces are (with the exception of case (i) below) entirely determined by the omitted element $d$ and its two adjacent neighbors $d'$ and $d''$, and we refer to such a halfspace by the triplet $d'<d<d''$.  The different types of boundary facets of $\Sigma_{(\delta,\epsilon)}$ that arise are the following:
\begin{enumerate}[(i)]
...
%	\[
%	(\dots, \emptyset, -2, 0, \delta^\zp, \dots)<(\dots, -2-\epsilon,-2, 0, \infty^\zp, \dots)<(\dots, -1-\epsilon,-2, 0, \infty^\zp, \dots).
%	\]
%Third, we could have $d_i=d'_i+1$ and $d<d''$ a shift from column $i$. For example, if $n=1$, we could have
%	\[
%	(\dots, \emptyset, -2, 0, \delta-1^\zp, \dots)<(\dots, \emptyset, -2, 0, \delta^\zp, \dots)<(\dots, -2-\epsilon,-2, 0, \infty^\zp, \dots)	\]
%Fourth, we could have $d'<d$ increases the last finite entry and then $d<d''$ is a codimension shift.  For instance,
%\[
%(\dots,\emptyset,-2,0,\delta-1^{\zp},\infty,\dots)<(\dots,\emptyset,-2,0,\delta^{\zp},\infty,\dots)<(\dots,\emptyset,-2,0,\infty^\zp,\dots).
%\]
%
...
(\dots,, 0,1^\zp,\dots) <(\dots, 0,2^\zp,\dots) <(\dots, 1,2^\zp,\dots) 
	...
	\item A chain where $d'<d$ is a homological shift from column $i$ and $d<d''$ is a homological shift from column $i-1$.  For example, the chain:
		...
		\]
	\item A chain where $d'<d$ is a codimension shift and $d<d''$ is also a codimension shift.  For example, the chain:
\[
(\dots,\emptyset,0^\zp,\delta+1,\delta+2,\infty,\dots)<(\dots,\emptyset,0^\zp,\delta+1,\infty,\dots)<(\dots,\emptyset,0^\zp,\infty,\dots).
\]
	\item   A chain where $d'<d$ is a codimension shift and $d<d''$ is a homological shift.  For example, the chain:
\[
(\dots,\emptyset,0^\zp,\delta+1,\delta+2,\infty,\dots)<(\dots,\emptyset,-1-\epsilon,0^\zp,\delta+1,\infty,\dots)<(\dots,\emptyset,-1-\epsilon,0^\zp,\infty,\dots).
\]
	\item   A chain where $d'<d$ is a homological shift and $d<d''$ is a codimension shift.  For example, the chain:
\[
(\dots,\emptyset,0^\zp,\delta+1,\delta+2,\infty,\dots)<(\dots,\emptyset,0^\zp,\delta+1,\infty,\dots)<(\dots,\emptyset,-1-\epsilon,0^\zp,\infty,\dots).
\]
\end{enumerate} 
...
The corresponding functionals are obtained as follows.  Throughout the remainder of the proof, it will be convenient to fix a shifted pure resolution $\FF_{\dd}$ of type $\dd$ for every shifted degree sequence $\dd$.
...
We next consider case (iv).  We set $c=\codim(d)$ and we shift homological indices so that $d_j\in \ZZ$ if and only if $j\in \{0, \dots, c\}$.
We fix the root sequence $f=(-d_{1}>-d_{2}>\dots>-d_{c-1})$ and we let $\cE_f$ be any supernatural bundle of type $f$.   The corresponding functional is:
\[
\beta(\FF)\mapsto \chi_{1,-\delta-c}(\FF\cdot \cE_f).
\]
If $\FF_d$ is a pure complex of type $d$, then there exists some $N$ such that:
\[
\FF_d\cdot \cE_f=\left[ \overset{\zp}{A^N}(-\epsilon)\gets A^N(-\delta-c) \right],
\]
and hence the functional evaluates to $N$.  
Now, let $e$ be any degree sequence in $P_{(\delta,\epsilon)}$.  As in the proof of cases (iii) and (iv) above, the cases $e\geq d''$ and $e\leq d'$ split in half. 
If $e\leq d'$, then $\FF_e\cdot \cE_f$ has the Betti table of a complex with finite length homology and, by Lemma~\ref{lem:pure and supernatural}, it is supported entirely in the ``upper region'' defined by $\chi_{1,-\delta-c}$.  If $e\geq d''$, then $\beta(\FF_e\cdot \cE_f)$ is supported entirely in the ``lower region''.  
%homological degrees $<1$, and so the functional also evaluates to $0$.

\daniel{This paragraph is where I stopped double checking the claims on my last pass through.}
Cases (v)--(vii) may all be treated simultaneously.  We let $c$ be the codimension of $d$.  By reindexing the columns, it suffices to consider the consider where the finite entries of $d$ lie in homological positions $\{0,\dots,c\}$.  We then let $f$ be the root sequence $f=(-d_0>\dots >-d_{c-2})$ and we let $\cE_f$ be a supernatural sheaf of type $f$.  In all three of these cases, the appropriate functional has the form:
\[
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
In this short section we illustrate the results from the previous sections via a few examples.  We also apply Theorem~\ref{thm:extremal rays refined} to prove Corollary~\ref{cor:monads}, which provides a decomposition result for the Betti tables of monads.
...
We conclude this section with an application on decomposing the Betti tables of monads.  See Example~\ref{ex:monads} above for an example of this result.
...
\end{cor}
\begin{proof}
Note that $\beta(\FF)$ is compatible with the codimension sequence $\bb'=(\dots,0,0,0^{\zp},\infty,\dots)$. We set $D'$ to be the diagram obtained from $\beta(\FF)$ as follows: we decompose $\beta(\FF)$ according to the decomposition algorithm induced by the simplicial structure on $\BBQ^{\bb'}(S)$, and we keep track of the partial sums along the way.  At a certain point in the algorithm we will have exhausted all of the codimension $1$ degree sequences.  We continue the algorithm, using codimension $0$ degree sequences of the form $\bb'$, until our partial sum $D'$ satisfies
\[
\sum_{i,j} (-1)^i\beta_{i,j}D'=\rank \cF.
\]
We stop the decomposition algorithm at this point.  By construction, we have a diagram $D'$ that is the sum of pure Betti tables that are compatible with the codimension sequence $\bb'$.  It follows that there exists scalar $\lambda'$ and a resolution $\FF'$ such that $D'=\lambda'\beta(\FF')$.

We next consider the dual Betti table $\beta(\Hom(\FF,S))$.  We note first that $\Hom(\FF,S)$ is compatible with the codimension sequence $\bb''=(\dots,0,0,0^{\zp},n+1,n+1\dots)$.  We then set $D''$ to be the diagram rom $\beta(\Hom(\FF,S))$ as follows: we decompose $\beta(\Hom(\FF,S))$ according to the decomposition algorithm induced by the simplicial structure on $\BBQ^{\bb''}(S)$, and we keep track of the partial sums along the way.  At a certain point in the algorithm we will have exhausted all of the codimension $1$ degree sequences.  We stop the algorithm at this point and set $D''$ to be the resulting partial sum.
By construction, $D''$ is the sum of pure Betti tables that are compatible with the codimension sequence $(\dots,\emptyset,0^\zp,n+1,\dots)$.  It follows (see Example~\ref{ex:sheaf resolutions}) that there exists a scalar $\lambda''$ and a free complex $\FF''$ where $\widetilde{\FF''}$ is  a $\cO_{\PP^n}$-resolution and and where $D''=\lambda'\beta(\FF'')$.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
In this section, we use our new foundation for Boij--S\"oderberg theory over $S$ to analyze cones of Betti tables of more general $\NN$-graded rings.  Throughout this section, our setup will be as follows.  Let $f: X\to \PP^n$, with $\dim(X)=d$ and $L=f^*\cO(1)$.  We also let $R=R(X,L)=\oplus_{e\in \mathbb N} H^0(X,L^{\otimes e})$ be the section ring of $L$.  Note that $R$ is an $S$-algebra.
...
\end{prop}
\begin{proof}
Let $\FF\in \DD^b(S)$ be a free complex that is compatible with $\bb$.  Since $\kk$ is infinite and $\FF$ is bounded, we may apply \cite[Theorem]{miller-speyer} and replace $\FF$ by a general translate such that $\FF$ is homologically transverse to $R$.  \daniel{If $f$ is not finite, then we can't actually apply the Miller--Speyer result, since $R$ won't be a finitely $S$-module. I suspect this is fixable, but for the moment, this is a gap.}Since $\FF$ is a free complex, we have $Lf^*\FF=f^*\FF=\FF\otimes_S R$.   We then consider the spectral sequence:
\[
...
\begin{remark}
In the definition of the map of cones given by $Lf^*$, the homologically transverse assumption is crucial.  For instance, let $f: X\to \PP^2$ be the blow-up of a point $P$, and let $\FF$ be the free resolution of homogenous coordinate ring of $P$.  Let $\bb:=(\dots,\emptyset, 2^\zp,\infty,\dots)$.  Then $\FF$ is compatible with $\bb$.  However $\HH_0(Lf^*\FF)$ is supported on the exceptional divisor, so $Lf^*\FF$ is not compatible with $\bb$.
\end{remark}

As a map of vector spaces, $Lf^*\colon \VV\to \VV$ 
%and $Rf_*\colon \WW\to \WW$ are 
is simply the identity map.  Hence, when $\kk$ is infinite, we immediately obtain the inclusion:
\[
\BBQ^{\bb}(S)\subseteq \BBQ^{\bb}(R) 
%\quad \text{ and } \quad \CQ^k(X,L)\subseteq \CQ^{k-d}(\PP^n).
\]
When $f$ is finite, we also obtain an inclusion $\CQ^k(X,L)\subseteq \CQ^{k-d}(\PP^n)$ given by $\gamma(\cE)\mapsto \gamma(f_*\cE)$.  
Further, in the case the maps $Lf^*$ and $f_*$ interact well with the functors $\Phi$ and $\Phi_X$.
...
\end{lemma}
\begin{proof}
We first note that $\beta(Lf^*\FF)$ equals $\beta(\FF)$ and, since $f$ is finite, $\gamma(\cE)$ equals $\gamma(f_*(\cE))$.  Theorem~\ref{thm:betti numbers of pairing} then provides a closed formula for the Betti numbers of $\Phi(\FF,f_*\cE)$ in terms of $\beta(\FF)$ and $\gamma(f_*(\cE))$.  As noted in the proof of Theorem~\ref{thm:PhiX}\eqref{thm:PhiX:1}, the same formula relates the Betti numbers of $\Phi_X(f^*\FF,\cE)$ with  $\beta(\FF)$ and $\gamma(f_*(\cE))$.
\end{proof}
...
For instance, since $R$ is $2$-dimensional, Example~\ref{ex:1441} implies that (up to scalar multiple), there is a minimal, exact complex
\[
...
\begin{remark}
Eisenbud and Scheyer have conjectured that every projective scheme admits an Ulrich sheaf~\cite[p. 543]{eis-schrey-chow}.  Corollary~\ref{cor:isom cones} provides the possibility of producing a counterexample.  Namely, imagine that for some $f\colon X\to \PP^n$ finite, we can produce a free complex $\FF$ of $R=R(X,L)$ modules such that $\beta(\FF)\in \BBQ^{d+1}(R)$ but where $\beta(\FF)$ does does not decompose as a sum of shifted pure tables of codimension $d+1$.  This would imply that $\BBQ^{d+1}(R)\supsetneq \BBQ^{d+1}(S)$ and hence, by the contrapositive of Corollary~\ref{cor:isom cones}, this would imply that $X$ does not admit an Ulrich sheaf.
\end{remark}
...
$$
where $\FF\in \DD^b(R) , \cE\in \DD^b(X)$ and where $p_2: X\times \AA^{m}\to \AA^{m}$ is the projection. We  may write
$\FF\cdot \cE$ for $\Phi'(\FF,\cE)$.
\end{defn}
...
We define $\BBirr(R)$ as the cone of multigraded Betti tables of complexes with irrelevant, and similarly for $\BBirr(C)$.  We may also define $\CQ^0(X)$ as the cone of multigraded cohomology tables on $X$.  By Theorem~\ref{thm:Phimulti}, the functor $\Phi'$ induces a bilinear pairing:
\[
\BBirr(R)\times \CQ^0(X)\to \BBirr(C),
\]
similar to the pairing of cones in Figure~\ref{fig:bracket}.  

The cone $\BBirr(C)$ admits many nonnegative functionals similar to the $\chi_{i,j}$ constructed earlier.  Namely, let $\alpha\in \ZZ^m$ be any multi-degree.  We let $\preceq$ be any total order of $\ZZ^m$ that refines the natural termwise partial order.  We then define $\chi_{i,\alpha}: \DD^b(C)\to \QQ$ by the formula
\[
