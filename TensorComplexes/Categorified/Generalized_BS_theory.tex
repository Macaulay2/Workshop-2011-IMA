...
\end{tabular}
\end{equation}
where $(\beta(\FF),\gamma(\cE))\mapsto \beta(\Phi(\FF,\cE))$.  By extending this pairing linearly, we obtain a bilinear pairing among the three positive, rational cones spanned by the tables from \eqref{eqn:**}.  This is
...
\end{enumerate}
\end{thm}
This result is essentially due to~\cite{eis-schrey1}: one of their main results is a similar statement for free resolutions, and, given our construction of $\Phi$, their proof may be adapted to the case of more general complexes. See \S\ref{sec:duality} for corresponding results with less restrictions on the codimension of the complex $\FF$.
