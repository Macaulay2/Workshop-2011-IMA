...
Let $G$ be a minimal set of generators for the semigroup
$\NE(X)$.  The \defi{irrelevant ideal of $C$}, denoted $\irr(C)$ is by definition the ideal
$
\irr(C)=\cap_{g\in G} (t^g).
$

We next define a pairing $\Phi_{X}: D^{b}(R) \times D^{b}(X) \to D^{b}(C)$ that is analogous to
$\Phi$. and we will define functionals $\chi_{i,\alpha}$ on $D^{b}(C)$ that are analogous to the 
$\chi_{i,j}$. In this section we will prove Theorem~\ref{thm:Phimulti} and a positivity result for the functionals $\chi_{i,\alpha}$.
...
Let $G$ be a minimal set of generators for the semigroup $\NE(X)$.  

We claim that if a multigraded complex $\Gbull$ on $C$ is exact over the generic point of $\AA^m$, then $\Gbull$ has irrelevant homology.  Indeed, if $\Gbull$ is exact over the generic point of $\AA^m$, then, since $\dim(C)=\rank \Pic(X)$, the homology of $\Gbull$ is supported on the vanishing of $t^\alpha$ for some $\alpha\in \Pic(X)$.  However, any such $t^\alpha$ lies in $\irr(C)$ by construction.
...
on the cone $\BBirr(C)$. Thus if $\cE$ is a vector bundle on $X$ and $\FF$ is a complex of free
$\Pic(X)$-graded 
