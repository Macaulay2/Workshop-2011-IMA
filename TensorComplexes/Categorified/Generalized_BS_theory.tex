...
\end{enumerate}
\end{thm}
One of the main results of~\cite{eis-schrey1} is a similar statement for free resolutions. Given our construction of $\Phi$, the introduction of $D^{b}(A)$ both simplifies the statement and enables us to extend their arguments to  more general cases.  See \S\ref{sec:duality} for results with less restrictions on the codimension of the complex $\FF$.
...
In place of the ring $A=\kk[t]$, we define $C$ as the semigroup ring $C=\kk[\NE(X)]$, where $\NE(X)\subseteq \Pic(X)$ is the subsemigroup of effective divisors.  %Given $\alpha \in \Pic(X)$ we write $t^\alpha$ for the corresponding element of $C$.  
The ring $C$ is also naturally $\Pic(X)$ graded.

Let $\DD^b(R)$ and $\DD^b(C)$ denote the bounded derived categories of finitely generated, multigraded $R$-modules and $C$-modules, respectively.   For $\FF\in \DD^b(R)$ or in $\DD^b(C)$, $i\in \ZZ$, and $\alpha\in \Pic(X)$, we define the multigraded Betti numbers by the formula $\beta_{i,\alpha} \FF:=\dim \Tor_i(\FF,\kk)_{\alpha}$.  Similarly, for $\cE\in \DD^b(X), i\in \ZZ$ and $\alpha\in \Pic(X)$, we define multigraded cohomology numbers by the formula $\gamma_{i,\alpha} \cE:=\dim H^i(X, \cE(\alpha))$.  Given $\FF\in \DD^b(R)$, we use $\widetilde{\FF}$ to denote the corresponding complex of coherent sheaves in $\DD^b(X)$.  

The Cox ring $R$ of $X$ comes equipped with a natural irrelevant ideal, and we say that a complex $\FF$ over $R$ or $C$ has \defi{irrelevant homology} if its homology is supported on the irrelevant ideal.  (See Definition~\ref{defn:irrC} for the irrelevant ideal of $C$).
...
No multiple of the Betti table $\beta(\FF)$ can equal the Betti table of a minimal, free complex with finite length homology. Indeed, such a complex would be a resolution,  so this case is covered by the theory of \cite{eis-schrey1}. But it is instructive to see how this follows directly from Theorem~\ref{thm:Phi}.
...
\begin{remark}
We refer throughout to the definition of $\langle -, -\rangle_{\tau,\kappa}$ given in \cite{eis-schrey-icm}.  Though the same functionals appear in \cite{eis-schrey1}, they are indexed differently.
\end{remark}
...
the table indicates how $\chi_{i,j}$ separates a Betti table into two regions.  In the upper region, this is simply computing an Euler characteristic, and in the lower region, it is dot product with the zero matrix. The functionals $\langle - , \cE \rangle_{\tau,\kappa}$ used previously in Boij-S\"oderberg theory can be expressed in terms of the $\chi_{i,j}$.   (Although the following proposition only treats
...
$$
\cc=(\dots, c_{-1}, c_{0}^{\circ}, c_{1}, \dots )
$$
where  
$$
...
Also, it follows from the main result of \cite{WMACE}\daniel{what citation?} that if a complex
$$
...
%\end{bmatrix}.
%\]
%The boldfaced entries indicate the shape of minimal pure diagram that is compatible with $\cc$ and that could contribute to $\beta(\FF)$, and this corresponds to the degree sequence $(\dots,\emptyset^\zp,2,3,4,6,\infty,\dots)$.   This yields:
\[
\begin{bmatrix}
1^\zp&-&-&-&-\\
-&\mathbf{5}&\mathbf{4}&\mathbf{1}&-\\
...
%and we set $\widetilde{\cc}=(c_{\delta},c_{\delta+1},\dots, c_{\epsilon})$ to be a truncation of $\cc$.  
...
\begin{proof}
Throughout this proof, we take the convention that, for any degree sequence $\dd$, $F_{\dd}$ denotes some pure complex of type $d$.

Note that $\beta(\FF)$ is compatible with the codimension sequence $\cc'=(\dots,0,0,0^{\zp},n+1,\dots)$. We may thus decompose $\beta(\FF)$ according the decomposition algorithm induced by the simplicial structure on $\BBQ^{\cc'}(S)$.  There then exists a collection of nonnegative $a_i\in \QQ_{\geq 0}$ and a chain of degree sequences $\dd^0<\dd^1<\dots <\dd^s$, such that each $\dd^i$ is compatible with $\cc'$ and such that
\[
\beta(\FF)=\sum_{i=0}^s a_i\beta(\FF_{\dd^i}).
\]
Since the $\dd^i$ form a chain, there is some maximal $r$ such that the degree sequence $\dd^r$ has codimension $>0$.  We define $D':=\sum_{i=0}^r a_i\beta(\FF_{\dd^i})$.  For $i\ne 0$, we can describe the Betti numbers $\beta_{i,j}D'$ explicitly:
\[
...
\end{cases}
\]
%Note that $D'$ is supported entirely in nonnegative homological degrees.  In facte may thus choose a By definition, we then have that the Betti table $\beta(\FF)-D'$ equals $0$ in all strictly positive homological degrees.
...
As above, we can describe the Betti numbers $\beta_{i,j}D''$ explicitly:
\[
\beta_{i,j}D''=\begin{cases}
\beta_{-i,j}\FF &\text{ if } i>0\\
0 & \text{ if } i<0.
...
We next claim that all entries of $E$ are nonnegative.  This follows from the method by which the decomposition algorithm proceed.  Within a single column, the decomposition algorithm works from top to bottom.  So $D'$ consists of the lowest degree syzygies from column $0$.  Since dualizing inverts degrees, $(D'')\vee$ consists the highest degree syzygies from column $0$.  Thus, the only possible problem would be if $D'$ and $(D'')^\vee$ overlapped in column $0$, and in this case, all entries of $E$ would be strictly negative.  This would contradict the computation in the previous paragraph.
...
& \geq c_r.
\end{align*}
Thus, $Lf^*\FF$ is compatible with $\cc$, as claimed.
\end{proof}
...
\end{lemma}
\begin{proof}
Since $\kk\subseteq K$ is flat, we have an obvious inclusion $\BBQ^{\cc}(R)\subseteq\BBQ^{\cc}(R\otimes_{\kk}K)$ induced by $\beta(\FF)\mapsto \beta(\FF\otimes_{\kk} K)$.  We seek to show the opposite inclusion.  We first deal with an algebraic extension and in this case it suffices to set $K=\overline{\kk}$, the algebraic closure.    Let $\FF\in \DD^b(R\otimes_{\kk}\overline{\kk})$ be a free complex that is compatible with $\cc$.  We claim that some scalar multiple of $\beta(\FF)$ lies in $\BBQ^{\cc}(R)$.  Note that $\FF$ is definable over a finite extension $\kk'$ of $\kk$.  Hence, if we set $R'=R\otimes_{\kk}\kk'$, then there is a bounded free complex $\FF'$ of $R'$-modules with $\beta(\FF')=\beta(\FF)$.  Since the extension $R\subseteq R'$ is finite and free of rank $[\kk':\kk]$, it follows that if $\FF''$ is the complex $\FF'$ viewed as an $R$-module, then $\beta^R(\FF'')=[\kk':\kk]\cdot \beta^{R'}(\FF')$.

It now suffices to handle the case of the purely transcendental extension $K=\kk(z)$.  Let $\FF\in \DD^b(R\otimes_{\kk}\kk(z))$ be a free complex that is compatible with $\cc$.  By clearing denominators, we may extend $\FF$ to a complex over $\kk[z][x_0,\dots,x_n]$.  The locus of $\Spec(\kk[z])$ where the specialization of $\FF$ fails to be compatible with $\cc$ is a closed subset of $\Spec(\kk[z])$.  Since $\FF$ is compatible with $\cc$ over the generic point, it follows that follows that we can choose a closed point $P\in \Spec(\kk[z])$ such that the restriction of $\FF$ to $P$ is still compatible with $\cc$.  Since the residue field of $P$ is finite over $\kk$, we are reduced to the case of a finite extension, and it follows from the above argument that $\beta(\FF)$ lies in  $\BBQ^{\cc}(R)$.
\end{proof}
...
\begin{example}[Embedded Curves]\label{ex:res over section ring of ell curve}
...
\begin{example}[$d$-gonal Curves]
Over an algebraically closed field $\kk$, let $C$ be a smooth curve of genus $g$ and let $f\colon C\to \PP^1$ be a finite map.  Then a general line bundle of degree $g-1$ is an Ulrich sheaf of $f$.  If $L=f^*\cO(1)$ and $R$ is the section ring $R=\oplus_{e\in \mathbb N} H^0(C,\cO_C(eL))$, then $\BBQ^2(R)=\BBQ^2(\kk[x_0,x_1])$.  Note that $R$ is generally a nonstandard graded ring in this case.
\end{example}
...
\end{cor}
\begin{proof}
Let $n=\dim(R)-1$.  For some $e\in \ZZ$, we use the notation $\FF_{\leq e}$ to denote the complex $\FF$ truncated in homological degrees $>e$.  I.e., we have:
\[
...
\]
where $a_k\in \QQ_{\geq 0}$, each $N_k$ is a finite length module of finite projective dimension, and where $N'$ has finite projective dimension.  We obtain this decomposition via the algorithm induced by the simplicial structure of $\BBQ^{\cc}(R)$.  Since this decomposition algorithm is a greedy algorithm that works from right to left, it follows that, as long as $k>-e$, the $a_k$ and the $\beta(N_k)$ will not depend on $e$.  Thus, as $e\to \infty$, we see that the decomposition will stabilize to an infinite sum
\[
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
In this section, we extend the pairing $\Phi$ to a toric variety and prove Theorem~\ref{thm:Phimulti}.  We then use this pairing to construct multigraded analogues of the Eisenbud--Schreyer functionals.  With $\chi_{i,\alpha}$ and $\Phi_X$ as defined below, we will prove the following nonnegativity result:


\begin{cor}\label{cor:chiialpha}
Let $X$ be a projective variety, $\cE$ be a vector bundle on $X$, and $\FF$ be a complex of free $\Cox(X)$-modules that is multigraded with respect to the $\Pic(X)$-grading and that has irrelevant homology.  Then, for any $i\in \ZZ$ and any $\alpha\in \Pic(X)$, we have
\[
...
We recall some notation from the introduction: $X$ is a projective toric variety and $R=\Cox(X)$ with the natural $\Pic(X)$ multigrading.
...
\begin{defn}\label{defn:irrC}
Let $G$ be a minimal set of generators for the semigroup $\NE(X)$.  We define the \defi{irrelevant ideal of $C$}, denoted $\irr(C)$ as
$
\irr(C)=\cap_{g\in G} (t^g).
$
\end{defn}

We  now define our pairing $\Phi_X$.  Set
$
\sigma: R\to R\otimes C
$
as the ring homomorphism given by $\sigma(f)=t^{\deg(f)}f$. 
We write $-\otimes_\sigma (R\otimes C)$ to denote tensoring over $R$ with $R\otimes C$
...
The grading group $\Pic(X)$ admits a natural partial order where $\alpha \geq \alpha'$ whenver $\alpha-\alpha'\geq \in \NE(X)$.  If we fix any total order $\preceq$ that refines this partial order, then we may define multigraded analogues of the $\chi_{i,j}$ functionals.

\begin{defn}
Let $i\in \ZZ$ and $\alpha\in \Pic(X)$.  We define $\chi_{i,\alpha}: \DD^b(C)\to \QQ$ by the formula
\[
...
We now consider statement \eqref{thm:Phi:2}. We claim first that if a multigraded complex $\Gbull$ on $C$ is exact over the generic point of $\AA^m$, then $\Gbull$ has irrelevant homology.  This follows from our definition of $\irr(C)$.  Namely, if $\Gbull$ is exact over the generic point of $\AA^m$, then, since $\dim(C)=\rank \Pic(X)$, the homology of $\Gbull$ is supported on the vanishing of $t^\alpha$ for some $\alpha\in \Pic(X)$.  However, any such $t^\alpha$ lies in $\irr(C)$ by construction.

It thus suffices to show that, if $\widetilde{\FF}\otimes \cE$ is exact, then $\Phi_X(\FF,\cE)$ is exact over the generic point of $\AA^m$.  Let $Q(C)$ be the fraction field point of $C$.  Then, the map $\sigma$ becomes the usual inclusion $R\subset R\otimes Q(C)$.
...
This opens up new possibilities in the study of Betti tables over $R$ and, dually, in the study of cohomology tables on $X$.
...
%\BBirr(R)\to \QQ_{\geq 0}
%\]
%given by $\beta(\FF)\mapsto \chi_{i,\alpha}(\FF\cdot \cE)$.  
