...
The Hilbert polynomial is a fundamental invariant of graded modules or coherent sheaves on projective space. This invariant is refined in two different ways by the Betti table of a graded module and the cohomology table of a coherent sheaf. Work of Eisenbud and Schreyer suggests a sort of complex numerical duality between these extensions. In this paper we show that his duality has a simple categorical explanation.
...
From this perspective, the functor $\tau$ induces a map $\tau\colon \DD^b([\AA^{n+1}/\GG_m])\to \DD^b(\PP^n\times [\AA^1/\GG_m])$.  Note that this map does not factor through the natural map $\DD^b([\AA^{n+1}/\GG_m])\to \DD^b(\PP^n)$ given by sending $\FF$ to the corresponding complex of sheaves $\widetilde{\FF}$.  For example, any complex $\FF$ with finite length homology would go to zero in $\DD^b(\PP^n)$, whereas $\tau(\FF)$ would be nonzero.
