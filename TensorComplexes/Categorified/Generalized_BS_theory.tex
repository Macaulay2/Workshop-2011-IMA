...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Throughout this section $X$ will denote a projective toric variety
with $Pic(X)$-graded Cox ring $R$ with irrelevant ideal $\irr(R)$. We write $\NE(X)\subseteq \Pic(X)$ for the subsemigroup of effective divisors, and write $C$ for the $Pic(X)$-graded semigroup ring $\kk[\NE(X)]$. When $\alpha\in \NE(X)$ we write $t^{a}$ for the corresponding element of $C$.
...
$\chi_{i,j}$ and we will prove Theorem~\ref{thm:Phimulti} and the following Corollary:

\begin{cor}\label{cor:chiialpha}
Let $X$ be a projective variety, $\cE$ be a vector bundle on $X$. If $\FF$ is a complex of free
$\Pic(X)$-graded 
$R$-modules that has irrelevant homology, then for any $i\in \ZZ$ and any $\alpha\in \Pic(X)$ we have
\[
...
$$
where $\FF\in \DD^{b}(R) , \cE\in \DD^b(X)$ and  $p_2: X\times \AA^{m}\to \AA^{m}$ is the projection. We sometimes write
...
We now consider statement \eqref{thm:Phi:2}. 
