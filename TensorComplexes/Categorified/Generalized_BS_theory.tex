...
\daniel{I'm not sure the following Corollary is helping.  But I just can't shake the feeling that $\BBQ^{n+1}(S)$ is a  distinguished special case.  The only place it currently shows up is in \S\ref{sec:duality}.}
...
As a corollary of this theorem (see Corollary~\ref{cor:monads} below), we can use shifted pure Betti tables to decompose the Betti table any monad.  
...
\begin{question}
Does there exist a counterexample to Corollary~\ref{cor:new graded rings} when $\kk$ is a finite field?  More generally, does there exist any example of a finite field $\kk$, a codimension sequence $\bb$, and a graded $\kk$-algebra $R$ such that extending the ground field enlarges the cone $\BBQ^{\bb}(R)$?\daniel{Second question may be too general.}
\end{question}
...
\]
with each $N_k$ a finite length module.  Setting $M_k:=N_k^\vee$ \daniel{get the twist right for the dual} and using the $a_k$ as above then yields the desired decomposition.
\end{proof}
%
%\begin{example}
