...
The functor $\Phi$ thus yields bilinear pairings relating Betti tables of complexes with homology to cohomology tables in a number of cases (\S\ref{sec:duality}).  For example, in the case of complexes with homology of finite length, we obtain a pairing
 \begin{equation}%\tag{**}
\label{eqn:**}
...
where $(\beta(\FF),\gamma(\cE))\mapsto \beta(\Phi(\FF,\cE))$.  By extending this pairing linearly, we obtain a bilinear pairing among the three positive, rational cones spanned by the tables the from \eqref{eqn:**}.  This is
 a \emph{duality pairing} of cones in the following sense:
...
\end{enumerate}
%A point $v\in \VV$ is a scalar The only elements of $\VV$ that pair with all elements of the cone in the second factor
%Fix a point $v\in \VV$ and an integer $1\leq k\leq n+1$.  Then $v$ lies in the rational cone spanned by Betti tables of complexes of codimension $k$ if and only if $v$ pairs with all cohomology
%pair with all elements of the cone 
\end{thm}
This result is essentially due to~\cite{eis-schrey1}: their main results imply a similar statement for free resolutions, and their proof may be adapted to the case of more general complexes.  We use the functor $\Phi$ to sharpen the statement and proof of the theorem.  %See \S\ref{sec:duality} for more discussion of this duality.
...
Let $R\supseteq S$ be a finite extension of graded rings, and let
...
\[
\Phi_X: \DD^b(R)\times \DD^b(X)\to \DD^b(C)
\]
with the following properties:
\begin{theorem}\label{thm:Phimulti}
\begin{enumerate}
	\item\label{thm:Phi':1}  The multigraded Betti table of $\Phi_X(\FF,\cE)$ depends only on the multigraded Betti table of $\FF$ and the multigraded cohomology table of $\cE$.
	\item\label{thm:Phi':2}  If $\widetilde{\FF}\otimes \cE$ is exact, then $\Phi_X(\FF,\cE)$ has irrelevant homology.  This occurs, for instance, whenever $\FF$ is a complex with irrelevant homology and $\cE$ is a vector bundle.
\end{enumerate}
\end{theorem}



The functor $\Phi_X$ thus yields bilinear pairings relating multigraded Betti tables and multigraded cohomology tables, including a pairing of the form:
\begin{equation*}%\tag{**}
...
% then there is a unique $i$ such that $f_i\geq \kappa >f_{i+1}$.  \
...
%Using the notation of XXXX, the cohomology table of $\cE$ is a given by
%\[
%\gamma(\cE)=
%\bordermatrix{
%&&&&&&&&&\cr
%h^2&\dots &12&5&-&-&-&-&-&\dots\cr
%h^1&\dots&-&-&3&4&3&-&-&\dots\cr
%h^0&\dots&-&-&-&-&-&5&12&\dots
%}
%\]
\[
...
%
%We say that $i$ is a \defi{nondecreasing index} (resp.\ a \defi{nonincreasing index}) of $\cc$ if the sequence $(\cc_i,\cc_{i+1},\dots)$ is nondecreasing (resp.\ nonincreasing.)
\end{defn}
...
%Let $\FF=[\FF_0\gets \dots \gets \FF_p]$ be the minimal free resolution of a module of codimension $c$.  The complex $\FF$ is compatible with $\cc=(\dots, 0,c-1^\circ,c,c+5,\dots)$.  However, $\FF$ is not compatible with $\cc=(\dots,c+1^\circ,c+1,\dots)$.  
\end{example}
...
\end{defn}
%\begin{defn}\label{defn:deg compatible}
%Suppose that $\cc$ is codimension sequence 
%and ${\dd}$ is a degree sequence  of codimension $\ell$. Let $k$ be the smallest index
%such that $d_{k}\in \ZZ$. If $k$ is a nondecreasing index of $\cc$ (resp.\ a nonincreasing index of $\cc$), then we say that $\dd$ is \defi{compatible with $\cc$} if $\cc_k\leq \ell\leq \cc_{k+1}$  (resp.\ if $\cc_{k+\ell}\leq \ell\leq \cc_{k+\ell-1}$).
%\end{defn}
...
%\emph{not} equivalent to asking that $\FF_d$ be compatible with $\cc$. 
%For instance, let
...
% 
% 
% , whereas the degree sequence $\dd$ is not compatible with $\cc$.
%
...
\right\}.
\]
Thus, if $\FF\in \DD^b(S)$ satisfies $\codim \FF\geq k$, then $\beta(\FF)$ may be uniquely written as a rational sum of shifted pure resolutions of codimension $k$ whose degree sequences form a chain.%where the shifted degree sequence $\dd$ corresponds to the ray spanned by the shifted pure resolution of type $\dd$ of a Cohen--Macaulay module. 
\end{cor}
...
Let $\cc$ be a codimension sequence for $A$ and let $u\in \UU$.
 %be a point in the vector space spanned by $\BBQ^{\cc}(A)$.  
...
We first apply Theorem~\ref{thm:Phi} to show that $\beta(\FF)$ cannot equal the Betti table of a minimal, free complex with finite length homology.  To prove this, we choose $\cE$ to be a rank $8$ supernatural bundle of type $(0,-8)$.  
%Note that $\cE=\Sym_7(\Omega_{\PP^2}))\otimes \cO(\pm 13)$ works.
Then $\FF\cdot \cE$ would have the form
\[
...
\]
For any minimal, free complex of this form, the kernel of the map will contain a subsheaf of $A(-4)^{256}$ of rank at least $256-240=16$, and hence $\FF\cdot \cE$ cannot have finite length homology.  By Theorem~\ref{thm:Phi}\eqref{thm:Phi:2}, we conclude that $\beta(\FF)$ cannot be the Betti table of a complex with finite length homology.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
We next use our new foundation for \BS theory over $S$ to analyze the structure of bounded free complexes over more general $\NN$-graded rings.  Throughout this section, our setup is as follows.  We fix a graded ring $R$ that is a finite extension of $S=\kk[x_0,\dots,x_n]$.  In other words, $R$ may be any graded ring that admits a linear Noether normalization.  The inclusion $S\subseteq R$ induces a finite map $f\colon X=\Proj(R)\to \PP^n$ of $n$-dimensional projective schemes.  We set $L:=f^*\cO(1)$.

We define cohomology tables and Betti tables for the pair $(X,L)$ as follows. If $\cE\in \DD^b(X)$ then $\gamma(\cE)\in \WW$ is the table with entries $\gamma_{i,j}(\cE)=h^i(X,\cE\otimes L^{\otimes j})$.  For any $0\leq k \leq d$, we set $\CQ^{k}(X,L)$ to be the cone spanned by of cohomology tables $\gamma(\cE)$ with $\codim(\cE)\geq k$.  We define $\DD^b(R)$ as the bounded derived category of finitely generated, graded $R$-modules.  For $\FF\in \DD^b(R)$, we define the Betti table $\beta(\FF)$ by the formulas $\beta_{i,j}(\FF)=\dim \Tor_i(\FF,\kk)_j$.  If $\cc$ is a codimension sequence, then we define $\BBQ^{\cc}(R)$ in parallel with the definitions from \S\ref{sec:refined}.

The main result in this section is a proof that relating cones of Betti tables of $R$ to cones of Betti tables over $S$, at least when $X$ contains an Ulrich sheaf.  This implies Corollary~\ref{cor:isom cones}.
...
To prove this corollary, we will pull back complexes from $S$ and then use a variant of the pairing $\Phi$.  We begin by defining the pairing $\Phi_f$:
\[
...
\end{lemma}
\begin{proof}
Since the extension $\kk\subseteq \overline{\kk}$ is flat, we have an obvious inclusion $\BBQ^{\cc}(R)\subseteq\BBQ^{\cc}(R\otimes_{\kk} \overline{\kk})$.  We seek to show the opposite inclusion.  Let $\FF\in \DD^b(R\otimes_{\kk}\overline{\kk})$ be a free complex that is compatible with $\cc$.  We seek to show that some scalar multiple of $\beta(\FF)$ lies in $\BBQ^{\cc}(R)$.  Note that $\FF$ is definable over a finite extension $\kk'$ over $\kk$.  Hence, if we set $R'=R\otimes_{\kk}\kk'$, then there is a free complex $\FF'\in \DD^b(R')$ with $\beta(\FF')=\beta(\FF)$.  Since the extnension $R\subseteq R'$ is finite and free, it follows that $\FF'$ induces an element $\FF''\in\DD^b(R)$ such that $\beta(\FF'')=[\kk':\kk]\cdot \beta(\FF)$. 
\end{proof}

The final ingredient for our proof of Corollary~\ref{cor:new graded rings} is the following lemma, which shows how the functors $\Phi$ and $\Phi_f$ interact with the map $f$.
...
We then claim that there exists a map of cones $Lf^*\colon \BBQ^{\cc}(S)\to \BBQ^{\cc}(R)$ defined by $\beta(\FF)\mapsto \beta(Lf^*\FF)$ whenever $\FF$ is homologically transverse to $R$.  Since $R$ is a finite $S$-module, and since $\FF$ is bounded, we may apply \cite[Theorem, p.\ 335]{miller-speyer} and replace $\FF$ by a general translate such that $\FF$ is homologically transverse to $R$. 

Note that, as a map of vector spaces, $Lf^*\colon \VV\to \VV$ 
is simply the identity map.  
Hence, when $\kk$ is infinite, we obtain the inclusion:
$
...
\begin{remark}\daniel{I wrote this hastily based on our last conversation.  But I must admit that I didn't think deeply about a few aspects of the argument.}
...
For instance, we might let $C$ be the rational quartic curve $f\colon C\to \PP^3$ given by the linear series $\langle x_0^4,x_0^3x_1,x_0x_1^3,x_4^4\rangle$.  In this case, $R=\kk[x_0^4,x_0^3x_1,x_0x_1^3,x_4^4]$, with all generators having degree $1$.  Since $\cO_{\PP^1}(3)$ is an Ulrich sheaf for $f$, we thus have $\BBQ^2(R)=\BBQ^2(\kk[x_0,x_1])$.
\end{example}
...
\daniel{This example and the previous one are pretty repetitive.  I could drop either.}
...
\begin{defn} \label{defn:product1} The functor $\Phi_X: \DD^{b}(R)\times \DD^b(X) \to \DD^{b}(C)$ is given by:
$$
\Phi_X(\FF,\cE) = Rp_{2*} \bigl(\tau(\FF)\otimes_{X\times\AA^{m}} (\cE\boxtimes \cO_{\AA^{m}}) \bigr)
$$
where $\FF\in \DD^b(R) , \cE\in \DD^b(X)$ and where $p_2: X\times \AA^{m}\to \AA^{m}$ is the projection. We also write
$\FF\cdot \cE$ for $\Phi_X(\FF,\cE)$.
\end{defn}

We are now prepared to prove Theorem~\ref{thm:Phimulti}.
\begin{proof}[Proof of Theorem~\ref{thm:Phimulti}]
Statement \eqref{thm:Phi:1} follows by applying essentially the same proof as used for Theorem~\ref{thm:betti numbers of pairing} above.  We now consider statement \eqref{thm:Phi:2}. It suffices to show that the homology of $\Phi_X(\FF,\cE)$ is annihilated by
...
Using $p_2$ to denote the restriction of $p_2$ to $X\times U$, it follows by a spectral sequence computation that the complex $Rp_{2*}\Gbull$ has no homology.  By flat base change, this is equal to the restriction of $\Phi_X(\FF,\cE)$ on the open set $U\subseteq \Spec(C)$, and we see that the homology
of $\Phi_X(\FF,\cE)$ is annihilated by a power of $h$, as required.
\end{proof}


We define $\BBirr(R)$ as the cone of multigraded Betti tables of complexes with irrelevant homology, and similarly for $\BBirr(C)$.  We also define $\CQ^0(X)$ as the cone of multigraded cohomology tables on $X$.  By Theorem~\ref{thm:Phimulti}, the functor $\Phi_X$ induces a bilinear pairing:
\[
